...
# Interface Streamlit
st.set_page_config(page_title="Prédiction de maladie cardiaque", page_icon="❤️", layout="wide")
...
st.markdown("""
    ...
    <div class="big-font">
    Cette application utilise un modèle de machine learning pour prédire la présence de maladies cardiaques en fonction des informations du patient.
    </div>
...
if st.sidebar.button("🔮 Faire une prédiction", key="predict_button"):
    ...
    if prediction[0] == 1:
        st.error("🚨 Le modèle prédit une maladie cardiaque.")
    else:
        st.success("✅ Le modèle prédit l'absence de maladie cardiaque.")

    # Afficher les probabilités
    st.write(f"📉 Probabilité d'absence de maladie cardiaque : {prediction_proba[0][0]:.2f}")
    st.write(f"📈 Probabilité de présence de maladie cardiaque : {prediction_proba[0][1]:.2f}")
